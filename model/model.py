...
class parsingNet(torch.nn.Module):
    def __init__(self, size=(288, 800), pretrained=True, backbone='50', det_dim=(37, 10, 4), cls_dim=(8, 4), use_aux=False):
        super(parsingNet, self).__init__()

        self.size = size
        self.w = size[0]
        self.h = size[1]
        self.det_dim = det_dim # (num_gridding, num_anchors, num_of_lanes)
        self.cls_dim = cls_dim # (num_classes, num_of_lanes)
        self.use_aux = use_aux
        self.total_det_dim = np.prod(det_dim)
        self.total_cls_dim = np.prod(cls_dim)
        ...
        if self.use_aux:
            ...
            self.aux_combine = torch.nn.Sequential(
                ...
                torch.nn.Conv2d(128, det_dim[-1] + 1,1)
        ...
        self.det = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(2048, self.total_det_dim),
        )

        self.cls = torch.nn.Sequential(
            torch.nn.Linear(1800, 1024),
            torch.nn.ReLU(),
            torch.nn.Linear(1024, self.total_cls_dim),
        )
